...
Comprehensive performance optimization guide for React Native applications, designed for AI agents and LLMs. Contains 36 rules across 14 categories, prioritized by impact from critical (core rendering, list performance) to incremental (fonts, imports). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.

Source: [Vercel Labs react-native-skills](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-native-skills)
...
1. [Core Rendering](#1-core-rendering) — **CRITICAL**
   - 1.1 [Never Use && with Potentially Falsy Values](#11)
   - 1.2 [Wrap Strings in Text Components](#12)
2. [List Performance](#2-list-performance) — **HIGH**
   - 2.1 [Avoid Inline Objects in renderItem](#21)
   - 2.2 [List performance callbacks](#22)
   - 2.3 [Keep List Items Lightweight](#23)
   - 2.4 [Optimize List Performance with Stable Object References](#24)
   - 2.5 [Pass Primitives to List Items for Memoization](#25)
   - 2.6 [Use a List Virtualizer for Any List](#26)
   - 2.7 [Use Compressed Images in Lists](#27)
   - 2.8 [Use Item Types for Heterogeneous Lists](#28)
3. [Animation](#3-animation) — **HIGH**
   - 3.1 [Animate Transform and Opacity Instead of Layout Properties](#31)
   - 3.2 [Prefer useDerivedValue Over useAnimatedReaction](#32)
   - 3.3 [Use GestureDetector for Animated Press States](#33)
4. [Scroll Performance](#4-scroll-performance) — **HIGH**
   - 4.1 [Never Track Scroll Position in useState](#41)
5. [Navigation](#5-navigation) — **HIGH**
   - 5.1 [Use Native Navigators for Navigation](#51)
6. [React State](#6-react-state) — **MEDIUM**
   - 6.1 [Minimize State Variables and Derive Values](#61)
   - 6.2 [Use fallback state instead of initialState](#62)
   - 6.3 [Use Dispatch Updaters for State That Depends on Current Value](#63)
7. [State Architecture](#7-state-architecture) — **MEDIUM**
   - 7.1 [State Must Represent Ground Truth](#71)
8. [React Compiler](#8-react-compiler) — **MEDIUM**
   - 8.1 [Destructure Functions Early in Render](#81)
   - 8.2 [Use .get() and .set() for Shared Values with React Compiler](#82)
9. [User Interface](#9-user-interface) — **MEDIUM**
   - 9.1 [Measuring View Dimensions](#91)
   - 9.2 [Modern React Native Styling Patterns](#92)
   - 9.3 [Use contentInset for Dynamic ScrollView Spacing](#93)
   - 9.4 [Use contentInsetAdjustmentBehavior for Safe Areas](#94)
   - 9.5 [Use expo-image for Optimized Images](#95)
   - 9.6 [Use Galeria for Image Galleries and Lightbox](#96)
   - 9.7 [Use Native Menus for Dropdowns and Context Menus](#97)
   - 9.8 [Use Native Modals Over JS-Based Bottom Sheets](#98)
   - 9.9 [Use Pressable Instead of Touchable Components](#99)
10. [Design System](#10-design-system) — **MEDIUM**
   - 10.1 [Use Compound Components Over Polymorphic Children](#101)
11. [Monorepo](#11-monorepo) — **LOW**
   - 11.1 [Install Native Dependencies in App Directory](#111)
   - 11.2 [Use Single Dependency Versions Across Monorepo](#112)
12. [Third-Party Dependencies](#12-third-party-dependencies) — **LOW**
   - 12.1 [Import from Design System Folder](#121)
13. [JavaScript](#13-javascript) — **LOW**
   - 13.1 [Hoist Intl Formatter Creation](#131)
14. [Fonts](#14-fonts) — **LOW**
   - 14.1 [Use Expo Config Plugin for Font Loading](#141)
...
### 2.2 List performance callbacks
...
### 8.2 Use .get() and .set() for Shared Values with React Compiler
...
### 14.1 Use Expo Config Plugin for Font Loading
